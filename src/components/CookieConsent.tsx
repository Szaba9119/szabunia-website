...
export default function CookieConsent() {
	...
	useEffect(() => {
		const emit = () => {
			...
			window.dispatchEvent(
				new CustomEvent('cookie-banner-change', { detail: { height } }),
			);
		};
		emit();
		if (!visible) return;
	...
	return (
		...
		>
			{/* Karta jest wyśrodkowana jak główne bloki strony. Na telefonie ma
		  ciaśniejsze paddingi i odstęp, żeby mniej zasłaniać treść pod banerem.

		  Przy `md:max-w-xl` karta ma 576 px i pozostaje centralnie wyrównana. */}
			<div className='pointer-events-auto max-w-3xl mx-auto md:max-w-xl bg-white dark:bg-dark-card rounded-2xl border border-border dark:border-dark-border shadow-xl shadow-navy/10 dark:shadow-black/30 p-4 sm:p-5 md:p-6 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4'>
				<div className='flex-1 min-w-0'>
					<p className='text-[12px] sm:text-[13px] text-text-body dark:text-dark-text leading-snug sm:leading-relaxed'>
