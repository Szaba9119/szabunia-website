...
const META: Record<
	GalleryCategoryKey,
	{ label: string; sub: string; alt: string }
> = {
	portrety: {
...
const CURATED: Partial<Record<GalleryCategoryKey, string[]>> = {
	...
	].map((n) => `/images/galeria/portrety/${n}.jpg`),
...
export default function ServiceGalleryStrip({
	...
	if (VIDEO_CATEGORIES.includes(category)) {
		...
		const vids = ids
			? ids
					.map((id) => galleryVideos.find((v) => v.youtubeId === id))
					.filter((v) => !!v)
		...
		return (
			<Shell
				label={meta.label}
				sub={subProp ?? meta.sub}
				href={href}
				ctaLabel={ctaLabel}
				prominentSub={!!subProp}
              ...
              nie ma ani słowa. Jeden system dla wszystkich sekcji treści. */}
                ...
                (Marcin 04.08.2026: „chciałbym, żeby to zdanie bardziej wybrzmiało"). */}
						{prominentSub ? (
							...
							<p className='text-steel dark:text-dark-text-muted text-[14px] mt-1'>
								{sub}
							</p>
						)}
					</div>
              ...
              wymaga poprawienia tej liczby. */}
					{secondaryLink && (
						<div className='mt-1 flex justify-center'>
							<SecondaryLink href={secondaryLink.href}>
								{secondaryLink.label}
							</SecondaryLink>
						</div>
					)}
