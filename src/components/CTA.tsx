...
function validateField(name: string, value: string): string | undefined {
	if (name === 'name' && !value.trim())
		return 'Podaj imię i nazwisko lub nazwę firmy';
	if (name === 'email') {
...
export default function CTA({
	...
	const [formData, setFormData] = useState({
		...
		// Dodane 14.08.2026 razem z CTA „Zapytaj o ofertę".
	...
	const GENERIC_ERROR =
		'Coś poszło nie tak. Spróbuj ponownie lub napisz bezpośrednio na marcin@szabunia.pl';
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (res.ok) {
				...
				const body: { sent?: boolean } | null = await res
					.json()
					.catch(() => null);
				if (body?.sent !== false) {
					gtagEvent('contact_submit', {
						service: formData.service || '(brak)',
					});
				}
			} else {
			...
			<div className='max-w-6xl mx-auto'>
					...
					<div className='relative rounded-[20px] overflow-hidden bg-white border border-border dark:bg-dark-card dark:border-dark-border px-6 py-12 md:px-10 md:py-16'>
						...
						<div className='absolute inset-0 -z-0'>
							...
							<Parallax
								...
								direction='up'
								className='absolute top-[30%] left-[20%] w-[400px] h-[400px] bg-[radial-gradient(circle,rgba(37,99,235,0.14)_0%,transparent_70%)]'
							/>
							<Parallax
								distance={PARALLAX.base}
								direction='down'
								className='absolute top-[50%] right-[10%] w-[300px] h-[300px] bg-[radial-gradient(circle,rgba(37,99,235,0.08)_0%,transparent_70%)]'
							/>
						</div>
                ...
                */}
								<ul className='flex flex-wrap gap-x-4 gap-y-1.5 mb-8'>
									{[
										'Faktura VAT',
										'Poprawki: 2 tury foto / 3 wideo',
										'Licencja komercyjna bez limitu czasu',
										'Realizacje w całej Polsce i Europie',
									].map((t) => (
										<li
											key={t}
											className='flex items-center gap-1.5 text-[13px] text-steel dark:text-dark-text-muted'
										>
											<svg
												className='w-4 h-4 text-blue dark:text-blue-light flex-shrink-0'
												fill='none'
												viewBox='0 0 24 24'
												stroke='currentColor'
												strokeWidth={2.2}
								...
								<div className='space-y-4 mb-8'>
									...
									<div className='flex items-center gap-3'>
										...
										<div>
											...
											<p className='text-navy dark:text-white text-sm'>
												Poznań, Polska
											</p>
										</div>
									</div>
                        ...
                        wcześniej _gotcha szło na sztywno puste, więc nie filtrowało niczego. */}
										...
										<div className='mb-3'>
											<label
												...
												className='block text-[11px] text-steel dark:text-dark-text-muted font-barlow font-semibold uppercase tracking-wide mb-1.5'
											>
											...
											{fieldErrors.name && (
												<p
													id='name-error'
													role='alert'
													className='text-red-600 dark:text-red-400 text-[11px] mt-1'
												>
													{fieldErrors.name}
												</p>
											)}
                        ...
                        którzy jej nie mają. */}
										...
										<label className='flex items-start gap-2.5 mb-1.5 cursor-pointer'>
											<input
												...
												className='mt-0.5 w-6 h-6 rounded border-border dark:border-navy-light accent-blue flex-shrink-0'
											/>
											<span className='text-[11px] text-steel dark:text-dark-text-muted leading-relaxed'>
												...
												<a
													...
													target='_blank'
													rel='noopener noreferrer'
													className='text-blue dark:text-blue-light hover:text-navy dark:hover:text-white underline transition-colors'
												>
										...
										{fieldErrors.consent && (
											<p
												id='consent-error'
												role='alert'
												className='text-red-600 dark:text-red-400 text-[11px] mb-3'
											>
												{fieldErrors.consent}
											</p>
										)}
										...
										{error && (
											<p
												role='alert'
												className='text-red-600 dark:text-red-400 text-[12px] mb-3 text-center'
											>
												{error}
											</p>
										)}
										...
										<button
											type='submit'
											disabled={
												sending || (TURNSTILE_ENABLED && !turnstileToken)
											}
