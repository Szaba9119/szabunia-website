		...
		>
			<ol className='flex flex-wrap items-center gap-x-1.5 gap-y-1'>
				{items.map((c, i) => (
					...
					<li key={c.name} className='flex min-w-0 items-center gap-x-1.5'>
						{i > 0 && (
							<span
								aria-hidden='true'
								className='shrink-0 text-steel dark:text-dark-text-muted'
							>
								/
							</span>
						)}
						{c.href ? (
							...
							<span
								className='inline-flex min-w-0 items-center min-h-[24px] text-navy dark:text-white'
								aria-current='page'
							>
								{c.name}
							</span>
